...
const otelLogScope = "csort.ru/auth-service"

// wrapWriterWithOTLPJSONTap writes through next and mirrors each zerolog JSON line to OTLP.
...
func bodyFromZerologMap(m map[string]interface{}) string {
	...
	if method != "" && (urlFull != "" || path != "") && status > 0 {
		...
		return fmt.Sprintf("HTTP %s %s -> %d | %s", method, target, status, msg)
	}
